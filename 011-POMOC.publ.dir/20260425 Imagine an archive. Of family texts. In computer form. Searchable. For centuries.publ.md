```markdown
# Imagine archives. Of family texts. In computer form. Searchable. For centuries.

// I have started  
a subreddit  
	r/survival\_of\_our\_info  
and a GitHub organization  
	 named "digital-gardens"  
		… it is linked  
		in the subreddit

the reason
why the sub


//////// in some subreddits, i will omit this  
By the way,  
the GitHub Desktop app  
is accessible  
to anyone  
who can use,  
for example,  
Microsoft Word,  
or Google Docs,  
or ChatGPT,  
or Gemini.

And, by the way,  
is there anyone   
who likes  
prose styles with short lines	  
e.g. the Buckminster Fuller's  
Ventilated Prose? 
```





# Dear AI,

I am preparing  
a reddit post..

It is my dream   
for many months  
// related song: ... 

i wonder,  
where to post it ,  
first attempt,  
today  
… a subreddit  
for

- technical people  
  - with disabilities?  
  - digital gardening  
    - Obsidian?  
    - Zettelkasten?  
  - 





can you suggest some   
suitable subreddits





can you   
give me  
links to  
some related reddit posts  
that already exist

..









or should i ask 
an online AI assistant
for such requests in the future?


# // minor edits,

#   please keep the line breaks,

#   please keep the indents 

—-----—---------

# It's the weekend\!

# // how to use google docs

//A note The help about voice commands.   
// exists..paste from markdown

## // **i shouldnt forget \- to pay attention \- to my digestion**

—-----—---------

# // old, excluded lines

i should put begging rather to comments  
(I will give reddit gold)  
(I am a person  
with disabilities)

Do you like the idea?

my github org, "digital-gardens" , that is very related to this subreddit .. survival of our info  
[https://github.com/digital-gardens](https://github.com/digital-gardens)

The subreddit is

The GitHub org, 

is linked   
in the subreddit.  
